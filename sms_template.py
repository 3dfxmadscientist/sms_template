...
class sms_template(osv.osv):
    ...
    def default_get(self, cr, uid, fields, context=None):
        res = super(sms_template, self).default_get(cr, uid, fields, context)
        if res.get('model'):
    ...
    def render_template_batch(self, cr, uid, template, model, res_ids, context=None, post_process=False):
        """Render the given template text, replace mako expressions ``${expr}``
           ...
           :param int res_ids: list of ids of document records those sms are related to.
        """
        if context is None:
            context = {}
    ...
    def get_sms_template_batch(self, cr, uid, sms_template_id=False, res_ids=None, context=None):
        if context is None:
            context = {}
        if res_ids is None:
            res_ids = [None]
        ...
        if not sms_template_id:
            return results
        template = self.browse(cr, uid, sms_template_id, context)
        ...
        for res_id, lang in langs.iteritems():
            if lang:
                ...
                template = self.browse(cr, uid, int(sms_template_id), context)
    ...
    _columns = {
        ...
        'lang': fields.char('Language',
                            help="Optional translation language (ISO code) to select when sending out an sms. "
        ...
        'user_signature': fields.boolean('Add Signature',
                                         help="If checked, the user's signature will be appended to the text version "
                                              "of the message"),
        'sms_from': fields.char('From',
            help="Sender address (placeholders may be used here). If not set, the default "
                    "value will be the author's sms alias if configured, or sms address."),
        'sms_to': fields.char('To (sms)', help="Comma-separated recipient addresses (placeholders may be used here)"),
        'sms_server_id': fields.many2one('sms.smsclient', 'SMS Gateway', readonly=False,
                                          help="Optional preferred server for outgoing sms. If not set, the highest "
    ...
    def create_action(self, cr, uid, ids, context=None):
        ...
        for template in self.browse(cr, uid, ids, context=context):
            ...
            button_name = _('YKJ Send SMS (%s)') % template.name
    ...
    def unlink_action(self, cr, uid, ids, context=None):
        for template in self.browse(cr, uid, ids, context=context):
            try:
                print template.ref_ir_act_window
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        return super(sms_template, self).unlink(cr, uid, ids, context=context)
    ...
    def copy(self, cr, uid, id, default=None, context=None):
        ...
        default.update(
            ...
            ref_ir_value=False)
        return super(sms_template, self).copy(cr, uid, id, default, context)
    ...
    def send_sms(self, cr, uid, sms_template_id, res_id, force_send=False, raise_exception=False, context=None):
        """Generates a new sms message for the given template and record,
           and schedules it for delivery through the ``sms`` module's scheduler.

           :param int sms_template_id: id of the template to render
           :param int res_id: id of the record to render the template with
                              (model is taken from the template)
           :param bool force_send: if True, the generated sms.message is
                immediately sent after being created, as if the scheduler
                was executed for this message only.
           :returns: id of the sms.message that was created
        """
        if context is None:
            context = {}
        ...
        values = self.generate_sms(cr, uid, sms_template_id, res_id, context=context)
        if not values.get('sms_from'):
            raise osv.except_osv(_('Warning!'), _("Sender sms is missing or empty after template rendering. Specify one to deliver your message"))
    ...
    def get_sms_template(self, cr, uid, sms_template_id=False, record_id=None, context=None):
        return self.get_sms_template_batch(cr, uid, sms_template_id, [record_id], context)[record_id]

    def generate_sms(self, cr, uid, sms_template_id, res_id, context=None):
        return self.generate_sms_batch(cr, uid, sms_template_id, [res_id], context)[res_id]
